...
u8  __afl_edge_san_area_initial[MAP_SIZE];
u8* __afl_edge_san_area_ptr = __afl_edge_san_area_initial;
...
static void __afl_map_shm(void) {
  ...
  u8 *edge_san_id_str = getenv(SHM_ENV_EDGE_SAN_VAR);
  ...
  if (edge_san_id_str) {
    ...
    u32 shm_edge_san_id = atoi(edge_san_id_str);

    __afl_edge_san_area_ptr = shmat(shm_edge_san_id, NULL, 0);

    /* Whooooops. */

    if (__afl_edge_san_area_ptr == (void *)-1) _exit(1);

    /* Write something into the bitmap so that even with low AFL_INST_RATIO,
       our parent doesn't give up on us. */

    __afl_edge_san_area_ptr[0] = 1;
  }
...
void __afl_log_loc(u32 cur_loc, u32 xor, u8* map) {
	if (loc_trace_file && !(map[xor])) {
