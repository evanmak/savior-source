...
def id_range_update(afl_file, cov_paths):
    ...
    id_val = int(os.path.basename(afl_file).split(',')[0].split('_')[1])
...
def coverage_diff(cycle_num, fuzz_dir, cov_paths, afl_file, cov, cargs):
    ...
    if cargs.cover_corpus or cargs.coverage_at_exit:
        a_file = 'id_%d...' % cov_paths['id_min']
        b_file = 'id_%d...' % cov_paths['id_max']
        delta_file = 'id_[%d-%d]...' % \
...
def search_cov(cargs):
    ...
    with open(id_delta_file, 'r') as f:
        for line in f:
            line = line.strip()
            ### id_NNNNNN*_file, cycle, src_file, cov_type, fcn/line\n")
...
def import_test_cases(qdir):
    return sorted(glob.glob(qdir + "/id_*"))
...
def mkdirs(cov_paths, cargs):
    ...
    if create_cov_dirs:
        ...
        if cargs.cover_corpus or cargs.coverage_at_exit:
            cfile.write("# id_[range]..., cycle, src_file, coverage_type, fcn/line\n")
        else:
            cfile.write("# id_NNNNNN*_file, cycle, src_file, coverage_type, fcn/line\n")
        cfile.close()
...
def parse_cmdline():
    ...
    p.add_argument("--lcov-web-all", action='store_true',
            help="Generate lcov web reports for all id_NNNNNN* files instead of just the last one",
            default=False)
    ...
    p.add_argument("--afl-queue-id-limit", type=int,
            help="Limit the number of id_NNNNNN* files processed in the AFL queue/ directory",
            default=0)
