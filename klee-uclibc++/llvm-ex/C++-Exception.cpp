...
/**************************************************************************
                   C++ Exception Allocation and Deallocation
***************************************************************************/
...
// the inferace function for exception allocation
extern "C"
void* __cxa_allocate_exception(unsigned thrown_size) throw() {
  
  return __llvm_cxxeh_allocate_exception(thrown_size);
}
...
// the inferace function for exception deallocation
extern "C" void __cxa_free_exception(void *vptr) throw() {
  __llvm_cxxeh_free_exception(vptr);
}
...
void __llvm_cxxeh_throw(void *ObjectPtr, void *TypeInfoPtr,
  ...
  E->TypeInfo = (std::type_info*)TypeInfoPtr;
  // printf("tinfo: %p = %s\n", TypeInfoPtr, E->TypeInfo->name());
...
// for KLOVER to execute a llvm::unwind instruction
extern "C" void _Unwind_RaiseException(void* obj);

// inferace function for exception throwing
extern "C" void
__cxa_throw (void *obj, std::type_info *tinfo, void (*dest) (void *)) throw()
{
  __llvm_cxxeh_throw(obj, tinfo, dest);
  _Unwind_RaiseException(obj);
}

// obtain the C++ exception header from a basic exception 
llvm_cxx_exception* _get_exception_header(llvm_exception* ex) {
  llvm_cxx_exception *Ex = 
    (llvm_cxx_exception *) ((llvm_exception*)(ex) - 1);
  std::type_info *ThrownType = Ex->TypeInfo;
  return Ex;
}

/**************************************************************************
                   C++ Exception Selection
***************************************************************************/

// for llvm::eh_exception()
extern "C"
void* __get_last_uncaught_exception() throw() {
  void *EPtr = __llvm_eh_get_uncaught_exception();
  if (EPtr == 0) return 0;  // If it's not a c++ exception, it doesn't match!
  return _get_exception_header((llvm_exception*)EPtr);
}
...
// for llvm::eh_select(...)
extern "C" std::type_info* _llvm_select_exception(llvm_cxx_exception* Ex, 
						  void* personality, ...) {
  va_list args;
  // llvm_cxx_exception* Ex = (llvm_cxx_exception*) E;
  std::type_info* info;
  std::type_info* matched_info = NULL;
  // printf("here 1\n");
  va_start(args, personality);
  do {
    ...
    if (info) {
      bool found = false;
      // ugly hack-in now until I find out how to distinguish primitive type and object type
      if (*info == typeid('c') || *info == typeid(1) || 
	  *info == typeid((short)1) || *info == typeid((long)1))
	found = *(Ex->TypeInfo) == *info;
      else 
	found =  CXXExceptionISA(Ex, info);
      if (found) {
	matched_info = info;
#ifdef DEBUG
	printf("Found the matched type info!\n");
#endif
	break;
      }
    }
...
// the inferace function for exception catching
extern "C" void* __cxa_begin_catch (void *exc_obj_in) throw()
{
  return __llvm_cxxeh_begin_catch();
}
...
#ifdef DEBUG
  // printf("Exiting end_catch Ex=%p HandlerCount=%d!\n", Ex, E->HandlerCount);
#endif
}

// the inferace function for finishing exception catching
extern "C" void __cxa_end_catch () {
  void *Ex = __llvm_cxxeh_get_last_caught();
  __llvm_cxxeh_end_catch(Ex);
}
...
// the inferace function for normal termination
extern "C" void
__cxa_call_terminate(void* exc_obj_in) {
  __llvm_cxxeh_call_terminate();
}

extern "C" void
__cxa_call_unexpected(void* exc_obj_in) {
  void (*Handler)(void) = __unexpected_handler;
  if (__llvm_eh_has_uncaught_exception())
    if (void *EPtr = __llvm_eh_current_uncaught_exception_type(CXXException))
      Handler = ((llvm_cxx_exception*)EPtr - 1)->UnexpectedHandler;
  __unexpected(Handler);
}


// the inferace function for terminating unexpected exceptions
extern "C"
...
// the inferace function for terminating unexpected exceptions
// extern "C" void
// _ZSt9terminatev(std::terminate_handler handler) {
//   __terminate(handler);
// }

extern "C"
void __unexpected (std::unexpected_handler handler) {
  handler();
  std::terminate ();
}

/**************************************************************************
                   C++ Exception Re-throwing
***************************************************************************/
...
void __llvm_cxxeh_rethrow() throw() {
  ...
  if (E == 0)
    ...
    // terminate.
    //
    __cxxabiv1::__terminate(__terminate_handler);
...
// // the inferace function for exception rethrowing
// extern "C" void
