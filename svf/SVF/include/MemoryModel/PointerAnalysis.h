    ...
    inline void callGraphSCCDetection() {
        if(callGraphSCC==NULL)
            callGraphSCC = new CallGraphSCC(ptaCallGraph); 
				// CallGraphSCC is of type defined as SCCDetection<PTACallGraph*>
				// SCCDetection is a class defined in Util/SCC.h
				// PTACallGraph is sub class of GenericGraph, ptaCallGraph is an instance of PTACallGraph

        callGraphSCC->find(); // visit each unvisited root node
    }
