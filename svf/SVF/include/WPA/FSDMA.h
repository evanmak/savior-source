//===- FSDominator.h -- Dominator analysis based on Flow-sensitive pointer analysis---------------------//
//
...
#ifndef FSDOMINATORANALYSIS_H_
#define FSDOMINATORANALYSIS_H_
 ...
 */
// YY add new data structures, including
// YY a map for function dominator, a map for function post-dominator (not necessarily in this cl    ass), <func, vector<func>>
// YY a map for basic block dominator, a map for basic block post-dominator (not necessarily in t    his class)
typedef WPAFSSolver<SVFG*> WPASVFGFSSolver;
class FSDominator : public WPASVFGFSSolver, public BVDataPTAImpl {
...
public:
    ...
    /// Constructor
    FSDominator(PTATY type = FSSPARSE_WPA) : WPASVFGFSSolver(), BVDataPTAImpl(type)
    {
        svfg = NULL;
    ...
    static FSDominator* createFSDMA(llvm::Module& module) {
        if (fspta == NULL) {
            fspta = new FSDominator();
    ...
    //@{
    static inline bool classof(const FSDominator *) {
        return true;
    }
