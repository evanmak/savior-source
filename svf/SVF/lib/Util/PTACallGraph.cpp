...
void PTACallGraph::buildCallGraph(llvm::Module* module) {
    ...
    /// create nodes
		// module->begin return begin of function iteration
    ...
    for (Module::iterator F = module->begin(), E = module->end(); F != E; ++F) {
        for (inst_iterator II = inst_begin(*F), E = inst_end(*F); II != E; ++II) {
            ...
            if (isCallSite(inst) && isInstrinsicDbgInst(inst)==false) {
								// isInstrinsicDbgInst return true if it is an instric debug instruction
                if(getCallee(inst))
...
void PTACallGraph::addCallGraphNode(const llvm::Function* fun) {
    NodeID id = callGraphNodeNum; // callGraphNodeNum is initialized as 0
    PTACallGraphNode* callGraphNode = new PTACallGraphNode(id, fun); // generate a callGraphNode
    addGNode(id,callGraphNode); // add a map relation from id to callGraphNode
    funToCallGraphNodeMap[fun] = callGraphNode; // add a map relation from caller fun to callGraphNode
    callGraphNodeNum++;
}
 ...
 */
PTACallGraphEdge* PTACallGraph::hasGraphEdge(PTACallGraphNode* src, PTACallGraphNode* dst,PTACallGraphEdge::CEDGEK kind) const { // CEDGEK is a enum type
...
void PTACallGraph::addDirectCallGraphEdge(const llvm::Instruction* call) {
    ...
    PTACallGraphNode* caller = getCallGraphNode(call->getParent()->getParent()); // get basic block, and then get function, search funToCallGraphNodeMap according to func

    PTACallGraphNode* callee = getCallGraphNode(getCallee(call));

		// edge has direction, from caller to callee
		// as mentioned in other places, only one edge exists between caller and callee,
		// however, this edge contains a set of callsite
    if(PTACallGraphEdge* callEdge = hasGraphEdge(caller,callee, PTACallGraphEdge::CallRetEdge)) {
        callEdge->addDirectCallSite(call); // add another callsite to this call outedge from src
        addCallGraphEdgeSetMap(call,callEdge); // add callsite (inst) - callee pair to csToIdMap and idToCSMap. here id is totalCallSiteNum
    }
    else {
        ...
        addEdge(edge); // src and dst add outgoing edge and incoming edge
