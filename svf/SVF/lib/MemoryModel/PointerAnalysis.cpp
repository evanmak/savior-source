...
void PointerAnalysis::initialize(Module& module) {
    ...
    /// whether we have already built PAG
		// pag is a static member variable
    if(pag == NULL) {
        ...
        symTable->buildMemModel(module); // identify which is value sym and which is object sym
        ...
        if (!Graphtxt.getValue().empty()) {
            ...
            PAGBuilder builder;
            pag = builder.build(module); // build pag here
        }
    ...
    if(EnableThreadCallGraph)
        ...
        ptaCallGraph = new PTACallGraph(mod); // build a call graph, multiple calls from function A to function B are merged into one call edge
				// constructor will build a call graph, connect direct call edge only
				// ptaCallGraph is call graph used for pointer analysis
				// call graph will indirect call included will be built later
    callGraphSCCDetection(); 
		// this is an inline function 
		// assign callGraphSCC from ptaCallGraph by CallGraphSCC constructor
		// callGraphSCC is SCC for CallGraph
}
