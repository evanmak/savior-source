//===- FSDMA.cpp -- Sparse flow-sensitive pointer analysis------------//
//
...
FSDominator* FSDominator::fspta = NULL;

/*!
 * Initialize analysis
 */
void FSDominator::initialize(llvm::Module& module) {
    PointerAnalysis::initialize(module); // initialization of a pointer analysis, including building symbol table and PAG etc.
    ...
    //AndersenWaveDiff::releaseAndersenWaveDiff();
		...
		// YY find a way to make up FlowSensitiveStat in WPAStat.h
 ...
 * Start analysis
 */
void FSDominator::analyze(llvm::Module& module) {
    ...
    DBOUT(DGENERAL, llvm::outs() << analysisUtil::pasMsg("Finish Solving Constraints\n"));
		// YY now we already have a call graph with indirectcall included
		// YY do something here to fill two maps for dominator relationship in function level
		// YY do something here to fill two maps for dominator relationship in basic level
 ...
 * SCC detection
 */
NodeStack& FSDominator::SCCDetect()
{
    double start = stat->getClk();
 ...
 */
void FSDominator::processNode(NodeID nodeId) {
 ...
 * Process each SVFG node
 */
bool FSDominator::processSVFGNode(SVFGNode* node)
{
    double start = stat->getClk();
    bool changed = false;
 ...
 */
bool FSDominator::propFromSrcToDst(SVFGEdge* edge) {
    double start = stat->getClk();
    bool changed = false;
 ...
 */
bool FSDominator::propAlongDirectEdge(const DirectSVFGEdge* edge)
{
    double start = stat->getClk();
    bool changed = false;
 ...
 */
bool FSDominator::propagateFromAPToFP(const ActualParmSVFGNode* ap, const SVFGNode* dst) {
 ...
 * Not necessary if SVFGOPT is used instead of original SVFG.
 */
bool FSDominator::propagateFromFRToAR(const FormalRetSVFGNode* fr, const SVFGNode* dst) {
 ...
 */
bool FSDominator::propAlongIndirectEdge(const IndirectSVFGEdge* edge)
{
    double start = stat->getClk();
 ...
 */
bool FSDominator::propVarPtsFromSrcToDst(NodeID var, const SVFGNode* src, const SVFGNode* dst)
{
    bool changed = false;
 ...
 */
bool FSDominator::processAddr(const AddrSVFGNode* addr) {
    double start = stat->getClk();
 ...
 * Process copy node
 */
bool FSDominator::processCopy(const CopySVFGNode* copy) {
    double start = stat->getClk();
 ...
 */
bool FSDominator::processPhi(const PHISVFGNode* phi) {
    bool changed = false;
 ...
 * Process gep node
 */
bool FSDominator::processGep(const GepSVFGNode* edge) {
    double start = stat->getClk();
    bool changed = false;
 ...
 */
bool FSDominator::processLoad(const LoadSVFGNode* load) {
    double start = stat->getClk();
    bool changed = false;
 ...
 */
bool FSDominator::processStore(const StoreSVFGNode* store) {
 ...
 */
bool FSDominator::isStrongUpdate(const SVFGNode* node, NodeID& singleton) {
    bool isSU = false;
 ...
 * Update call graph
 */
bool FSDominator::updateCallGraph(const CallSiteToFunPtrMap& callsites)
{
    double start = stat->getClk();
 ...
 */
void FSDominator::connectCallerAndCallee(const CallEdgeMap& newEdges, SVFGEdgeSetTy& edges) {
 ...
 */
void FSDominator::updateConnectedNodes(const SVFGEdgeSetTy& edges)
{
 ...
 * Propagate points-to information of a certain variable from src to dst.
 */
bool FSDominator::propVarPtsAfterCGUpdated(NodeID var, const SVFGNode* src, const SVFGNode* dst)
{
    if (isa<StoreSVFGNode>(src)) {
