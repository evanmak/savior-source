...
void Andersen::analyze(llvm::Module& module) {
    /// Initialization for the Solver
		// it is an important method to understand relationship between node and IR statement
    initialize(module); // module is actually result of parseIRFile(InputFilename, Err, Context)

		// check if there already has an Andersen's analysis results file 
    bool readResultsFromFile = false;
    if(!ReadAnder.empty()) // ReadAnder is cl::opt
...
{
		// ConstraintGraph is a sub-class of GenericGraph, which is used for program representation
		// core data struct is a map
		// consCG is initialied in initialize function
    for (ConstraintGraph::const_iterator nodeIt = consCG->begin(), nodeEit = consCG->end(); nodeIt != nodeEit; nodeIt++) {
        ConstraintNode * cgNode = nodeIt->second; // type of NodeType*, that is NodeTy*, a type in template 
        for (ConstraintNode::const_iterator it = cgNode->incomingAddrsBegin(), eit = cgNode->incomingAddrsEnd(); // addressInEdges.begin(), addressInEdges.end(), all incoming address edge of this node
                it != eit; ++it)
            processAddr(cast<AddrCGEdge>(*it)); // for each node, for each edge "it"
						// AddrCGEdge stands for copy edge
    }
}
...
void Andersen::processAddr(const AddrCGEdge* addr) {
    ...
    if(addPts(dst,src)) // union/add points-to
