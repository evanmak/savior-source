...
// cl::opt is the class used to represent scalar command line options
...
void WPAPass::runPointerAnalysis(llvm::Module& module, u32_t kind)
{
    ...
    _pta->analyze(module);
		// YY after building a call graph in analyze, build a function-level dominator relation into twp map data structure
		// YY now you have a _pta, you can first get dominator relation in _pta
		// YY with api from llvm , it should be easy to build basic dominator relationship in signle function
		// YY below is useless
    if (anderSVFG) {
